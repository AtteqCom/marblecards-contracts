...
contract MarbleBank is MarbleBankInterface, Ownable {
   ...
   */
  function deposit(ERC20 token, uint256 amount, string memory note) override external {
   ...
   * @param note every transaction has its note
   */
  function withdraw(ERC20 token, uint256 amount, string memory note) override external {
   ...
   * @param note every transaction has its note
   */
  function pay(ERC20 token, uint256 amount, address to, string memory note) override external {
   ...
   * @param note every transaction has its note
   */
  function payByAffiliate(ERC20 token, uint256 amount, address from, address to, string memory note) override external mustBeAffiliate(msg.sender) {
   ...
   */
  function hasEnoughTokens(ERC20 token, uint256 amount, address user) override external view hasTokenAccount(user, address(token)) returns(bool) {
   ...
   */
  function userBalance(ERC20 token) override external view hasTokenAccount(msg.sender, address(token)) returns(uint256) {
   ...
   */
  function addAffiliate(address newAffiliate) override external onlyOwner {
   ...
   */
  function removeAffiliate(address affiliate) override external onlyOwner {
   ...
   */
  function isAffiliate(address testedAddress) override external view returns(bool) {
  ...
  function _deposit(address from, ERC20 token, uint256 amount, string memory note) private {
  ...
  function _pay(address from, address to, ERC20 token, uint256 amount, string memory note) private hasTokenAccount(from, address(token)) {
  ...
  function _withdraw(address user, ERC20 token, uint256 amount, string memory note) private {
  ...
  function _userBalance(address userAddress, ERC20 token) private view hasTokenAccount(userAddress, address(token)) returns(uint256) {
